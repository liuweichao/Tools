...
!(function () {
var PreViewImg = function (params) {
  var defaults = {
    file: 'preViewFile',
    compressWrap: 'compressWrap',
    base64Img: 'base64Img',
    /*
      Callbacks:
  ...
  var canvas = p.params.canvas = document.createElement('canvas')
  p.params.ctx = canvas.getContext("2d")
  var $compressWrap = $('.'+ p.params.compressWrap)
  canvas.width = $compressWrap.width()
  canvas.height = $compressWrap.height()

  p.readFile = function (e) {
     var file = e.target.files[0]
        ...
        function readImgAfter (data) {
           p.emit('onFileChange', p)
           var $previewImg = $('<img class="'+ p.params.base64Img +'" src="'+ data +'" alt="" />')
           $previewImg.css({
               'width': '100%',
               'max-width': 'none',
               'position': 'absolute',
               'left': 0,
               'top': 0
           }).appendTo('.'+ p.params.compressWrap)
          p.gesture($previewImg.get(0))
        }
  ...
  $('.'+p.params.file).on('change', p.readFile)

  p.compressBase64 = function (callback) {
    var ctx = p.params.ctx

    if (callback) {
      return callback(ctx)
    } else {
      ctx.clearRect(0, 0, p.params.canvas.width, p.params.canvas.height)
      $('.'+ p.params.compressWrap).find('img.base64Img').each(function() {
        var thatImg = this
        var newImage = new Image()
        newImage.setAttribute('crossOrigin', 'anonymous')
        newImage.src = $(thatImg).attr('src')
        var transform = thatImg.myTransform,
             x = transform.translate.x || 0,
             y = transform.translate.y  || 0,
             scale = transform.scale || 1,
             rotate = transform.angle || 0,
             w = thatImg.clientWidth,
             h = thatImg.clientHeight
         ctx.save()
      ...
      var base64 = p.params.canvas.toDataURL("image/png")
      return base64
    }
  }
...
/* 手势 */
  ...
  p.gesture = function gesture (el) {
      ...
      function onPan(ev) {
          if(ev.type == 'panstart') {
              START_X = transform.translate.x || 0
              START_Y = transform.translate.y || 0
          }
          transform.translate = {
              x: START_X + ev.deltaX,
              y: START_Y + ev.deltaY
      ...
      function onRotate(ev) {
          if(ev.type == 'rotatestart') {
              ...
              changeAngle = ev.rotation || 0
          }

          transform.rz = 1;
          transform.angle = initAngle + ev.rotation - changeAngle;
          // console.log("----------" + ev.type +"----------")
          // console.log("initAngle:" + initAngle)
          // console.log("ev.rotation:" + ev.rotation)
...
/*触发*/
  p.emit = function (eventName) {
      // Trigger callbacks
      if (p.params[eventName]) {
          p.params[eventName](arguments[1], arguments[2], arguments[3], arguments[4], arguments[5])
      }
  }
